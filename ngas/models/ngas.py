...
class NeuralGas:
    """Classical online Neural Gas with competitive Hebbian graph updates.

    Uses the original neighborhood weighting by rank:
        h_lambda(k) = exp(-k / lambda)
    where k is the rank (winner has k=0).

    Optional annealing can be enabled with (final_lr, lambda_end, decay_steps),
    using geometric interpolation across update steps.
    """
    ...
    def __init__(
        ...
        if final_lr is not None and final_lr <= 0:
            raise ValueError("final_lr must be > 0 when provided.")
        if lambda_start is not None and lambda_start <= 0:
            raise ValueError("lambda_start must be > 0 when provided.")
        if lambda_end is not None and lambda_end <= 0:
            raise ValueError("lambda_end must be > 0 when provided.")
    ...
    def _annealed_values(self) -> tuple[float, float]:
        lr_now = self.config.lr
        lambda_now = float(self.config.lambda_start)

        if self.config.decay_steps is None:
            return lr_now, lambda_now

        t = min(self._step, self.config.decay_steps)
        frac = float(t) / float(self.config.decay_steps)

        if self.config.final_lr is not None:
            lr_now = self.config.lr * ((self.config.final_lr / self.config.lr) ** frac)

        if self.config.lambda_end is not None:
            lambda_now = self.config.lambda_start * (
                (self.config.lambda_end / self.config.lambda_start) ** frac
            )

        return float(lr_now), float(lambda_now)
    ...
    def update(self, sample: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        lr_now, lambda_now = self._annealed_values()

        # Original Neural Gas neighborhood by rank.
        rank_positions = torch.arange(rank.numel(), device=self.device, dtype=self.dtype)
        influence = torch.exp(-(rank_positions / lambda_now)).unsqueeze(-1)

        sorted_weights = self.weights[rank]
        sorted_weights.add_(lr_now * influence * (sample - sorted_weights))
    ...
    @torch.no_grad()
    def fit(self, data: torch.Tensor, epochs: int = 1, shuffle: bool = True) -> "NeuralGas":
